...
\subsection{Kraftübertragungsmaxima} %TODO!!
Um eine Aussage über den Radius $r$ für eine optimale Kraftübertragung zu können muss herausgefunden werden unter welchen Bedingungen die Winkelbeschleunigung 

% Teil I: Definitionen und grundlegende Beziehungen
Die Winkelgeschwindigkeit $\omega$ des Krans nach dem Stoß ergibt sich aus der Bahngeschwindigkeit am Einschlagspunkt:\\
I:
\[
\omega = \frac{v_b'}{r}
\]

Dabei ist $v_b'$ die Bahngeschwindigkeit des Krans nach dem Stoß am Radius $r$.

Die effektive Masse beim Stoß hängt vom Trägheitsmoment $I$ des Krans und dem Radius $r$ ab:
\[
m_{eff} = \frac{I}{r^2}
\]

% Stoßgleichungen
Beim teilelastischen Stoß gelten zwei grundlegende Beziehungen. Die erste beschreibt den Zusammenhang zwischen den Geschwindigkeiten vor und nach dem Stoß über den Stoßkoeffizienten $k$:
\[
\left(v_{n,c} - v_b\right) \cdot k = v_b' - v_{n,c}'
\]

Da der Kran anfangs ruht, gilt $v_b = 0$. Die Gleichung vereinfacht sich zu:
\[
v_{n,c} \cdot k = v_b'-v_{n,c}'
\]

Da wir uns nicht für die resultierende Geschwindigkeit des Autos interessieren, kann diese Gleichung nach ihr umgestellt, und $v_{n,c}'$ später durch ihre alternative Darstellung ersetzt werden \\
II:
\[
v_{n,c}' = v_b'- v_{n,c} \cdot k
\]


% Teil II: Impulserhaltung
Die zweite fundamentale Beziehung ist die Impulserhaltung beim Stoß. Der Gesamtimpuls vor dem Stoß muss gleich dem Gesamtimpuls nach dem Stoß sein:
\[
v_{n,c} \cdot m_c + v_b \cdot m_c = v_{n,c}' \cdot m_c + v_b' \cdot m_{eff}
\]

Mit $v_b = 0$ wird daraus:\\
III:
\[
v_{n,c} \cdot m_c = v_{n,c}' \cdot m_c + v_b' \cdot m_{eff}
\]

II in III einsetzen:
Wir setzen nun die Beziehung für $v_{n,c}'$ aus der ersten Gleichung in die Impulserhaltung ein:
\[
v_{n,c} \cdot m_c =\left(v_b'- v_{n,c} \cdot k \right)  \cdot m_c + v_b' \cdot m_{eff}
\]

Ausmultiplizieren:
\[
v_{n,c} \cdot m_c = - v_{n,c} \cdot k \cdot m_c \; + \; v_b'\cdot m_c \cdot m_c + v_b' \cdot m_{eff}
\]
$v_b'$ Ausklammern

\[
v_{n,c} \cdot m_c = - v_{n,c} \cdot k \cdot m_c \; + \; v_b'\cdot \left( m_c + m_{eff}\right)
\]

$v_{n,c} \cdot k \cdot m_c$ Addieren
\[
v_{n,c} \cdot m_c \; + \; v_{n,c} \cdot k \cdot m_c = v_b'\cdot \left( m_c + m_{eff}\right)
\]
...
$m_c$ Ausklammern
\[
v_{n,c} \cdot m_c \cdot \left(1+k\right) = v_b'\cdot \left( m_c + m_{eff}\right)
\]
Auflösen nach $v_b'$:
\[
\frac{v_{n,c} \cdot m_c \cdot \left(1+k\right)}{ m_c + m_{eff}} = v_b'
\]

I einsetzen 
% Einsetzen von m_eff
Mit $m_{eff} = \frac{I}{r^2}$ ergibt sich:
\[
v_b' = \frac{v_{n,c} \cdot m_c \cdot (1 + k)}{k \cdot m_c + \frac{I}{r^2}}
\]

Mit $v_b' = \omega\cdot r$ ergibt sich:
\[
\omega\cdot r = \frac{v_{n,c} \cdot m_c \cdot (1 + k)}{k \cdot m_c + \frac{I}{r^2}}
\]

Mit $v_b' = \omega\cdot r$ ergibt sich:
\[
\omega= \frac{v_{n,c} \cdot m_c \cdot (1 + k)}{m_c + \frac{I}{r^2}}\cdot \frac{1}{r}
\]

Schrittweise Ausmultiplizieren:
\[
...
Um optimale Einschlagsposition zu finden, bei der die Winkelgeschwindigkeit maximal wird, $\omega$ nach unserer Gesuchten Größe $r$ ableitebn und setzen die Ableitung gleich null(Extremum finden):
\[
\frac{d\omega}{dr} = 0
\]

Für die Funktion $\omega(r) = \frac{v_{n,c} \cdot m_c \cdot (1 + k) \cdot r}{m_c \cdot r^2 + I}$ verwenden wir die Quotientenregel:
\[
\frac{d}{dr}\left(\frac{u}{v}\right) = \frac{u' \cdot v - u \cdot v'}{v^2}
\]

\[
\frac{d\omega}{dr} = \frac{v_{n,c} \cdot m_c \cdot (1 + k) \cdot (m_c \cdot r^2 + I) - v_{n,c} \cdot m_c \cdot (1 + k) \cdot r \cdot 2 \cdot m_c \cdot r}{(m_c \cdot r^2 + I)^2}
\]

Den gemeinsamen Faktor $v_{n,c} \cdot m_c \cdot (1 + k)$ im Zähler ausklammern:
\[
\frac{d\omega}{dr} = \frac{v_{n,c} \cdot m_c \cdot (1 + k) \cdot \left[(m_c \cdot r^2 + I) - r \cdot 2 \cdot m_c \cdot r\right]}{(m_c \cdot r^2 + I)^2}
\]

Die Klammer im Zähler vereinfachen:
\[
...
Dafür muss eine Zählernullstelle gefunden werden der Nenner $(m_c \cdot r^2 + I)^2$ immer positiv ist und der Vorfaktor $v_{n,c} \cdot m_c \cdot (1 + k)$ ebenfalls nicht null ist, muss der Zählerterm null werden:
\[
\underbrace{v_{n,c} \cdot m_c \cdot (1 + k)}_{>0} \cdot (I - m_c \cdot r^2) = 0
\]

Da $v_{n,c} \cdot m_c \cdot (1 + k) > 0$, folgt:
\[
I - m_c \cdot r^2 \overset{!}{=} 0
\]
